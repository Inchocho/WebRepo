...
<html>
	<head>
		...
		* {
		...
		/* 가상태그(:) after라는 태그가 존재한다
		#container의 자식으로 after라는 이름의 태그가 존재한다고 가정하고 작동*/
		#container:after {
			content: "";
			display: block;
			clear: both;
...
<!-- 	float을 해제하는 몇 가지 방법 -->
<!-- 	1. float된 박스들의 바로 다음 박스에 clear : both;를 적용한다 -->
<!-- 	바로 다음에 박스가 없으면 사용 할 수 없다 -->
	
<!-- 	4. float된 박스들을 감싸는 박스에 height값을 준다 -->
<!-- 	-본문 내용엔 높이를 주지 않으므로 세로 사이즈가 불변인 경우 아니면 사용불가 -->
<!-- 	(왠만하면 사용하지말자 변화에 대응못함) -->
